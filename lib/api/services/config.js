...
let configObj
  , reduceWithKeyAndObj = utils.reduceWithKeyAndObj;
...
function validateConfig(dirtyObj, dbSchemaObj) {
  return madonna.validateSternly(
    getConfigSchema(dbSchemaObj)
    , dirtyObj
  );
}
...
function configCustomizer(objVal, srcVal) {
  if (fp.isArray(objVal)) { // src will also be an array then
