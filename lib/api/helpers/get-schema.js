  ...
  return bGetDb(dbPath)
    ...
    .then(rows => {
      ...
      return bPromise.props({
        ...
        flat: bPromise.props(
          fp.reduce(
            (res, name) =>
              fp.set(name, db.allAsync(queries.getPragmaTable(name)), res),
            {},
    ...
    .then(({ flat, partitioned }) => {
      ...
      schema = fp.assign(
        ...
        fp.mapValues(
          fp.reduce(
            (res, name) => fp.set(name, tableAndViewToPragma[name], res),
            {}
          ),
          partitioned
        )
...
function booleansToFlags(val) {
  const possibleFlags = ['isNullable', 'isPrimaryKey'],
    flags = fp.intersection(possibleFlags, fp.keys(val))
