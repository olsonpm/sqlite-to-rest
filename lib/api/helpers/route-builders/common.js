...
const { isDefined, startsWith } = utils,
...
function parseQueryForPkColumns(str, pkColumnNames, qsErrIds) {
  ...
  while (str.length) {
    ...
    if (!col) {
      ...
      return {
        msg:
          parseErr +
          'Duplicate pk columns not allowed.\n' +
          'duplicate column: ' +
          col +
          appendMidParsed(str) +
          '\noriginal query: ' +
  ...
  if (fp.size(pkColumnsMissing)) {
    return {
      msg:
        parseErr +
        'All pk columns must be passed.\n' +
        'query: ' +
        originalQueryStr +
        '\nmissing pk columns: ' +
        pkColumnsMissing.join(', '),
      hasErr: fp.constant(true),
...
module.exports = {
  ...
  bRunQuery: bRunQuery,
  bStreamFinished: bStreamFinished,
