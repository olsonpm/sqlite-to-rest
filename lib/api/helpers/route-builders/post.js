...
const bRunQuery = common.bRunQuery
  , attachError = common.attachError
  , rbErrIds = errIds.post.requestBody
  , hasFlag = common.hasFlag
  ;
...
function getRequestBodyError(reqBody, columns) {
  ...
  if (fp.size(requiredColumnsMissing)) {
    return {
      msg: parseErr + "All non-nullable and non INTEGER PRIMARY KEY columns "
        + "must be passed.\n"
        + "missing columns: " + requiredColumnsMissing.join(', ')
        + "\npost request body: " + utils.jstring(reqBody)
...
function getRequiredColumns(columnNames, columns) {
  ...
  const isIntegerPk = cname => {
    return fp.includes('isPrimaryKey', columns[cname].flags)
      && fp.getOr('', cname + '.type', columns).toLowerCase() === 'integer';
  };

  return fp.reject(
    fp.anyPass([isNullable, isIntegerPk])
    , columnNames
  );
}
