...
const {
    appendMidParsed, attachError, bStreamFinished, getInvalidOrderElements
    , getParams, getQuery, quoteNotAdjacentToAnother, removeLeading
  } = common
  ...
  router.get('/' + name, (ctx, next) => {
    ...
    return connections.readOnly.getAsync(query, params)
      .then(({ count }) => {
        ...
        rangeRequest({connections, query, params, stringifyStream, range, count, ctx})
          .catch(err => {
            console.error(err);
          })
          .finally(() => {
...
function rangeRequest(scope) {
  ...
  return eachAsync(connections.readOnly, query, params, stringifyStream);
}
