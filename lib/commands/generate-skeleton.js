...
function generateSkeleton({ dir, dbPath }) {
  const installDeps = hasFileInDirectory('package.json', dir)
    ...
    .then(function() {
      ...
      return execAsync('npm i --save koa@^2 olsonpm/sqlite-to-rest', { cwd: dir });
    });

  const writeTpl = bPromise.props({
      tpl: bFs.readFileAsync(
        path.join(__dirname, 'resources/skeleton.tpl'), 'utf8'
      )
    ...
    .then(({ tpl, skeletonFName }) => {
      ...
      cliLog(
        'Writing the skeleton server to: ' + skeletonFName
      );
...
function hasFileInDirectory(fname, dir) {
  return bFs.readdirAsync(dir)
    .then(fp.includes(fname));
}

function getSkeletonFName(dir) {
  return bFs.readdirAsync(dir)
    .then(fileNames => {
      if (!fp.includes('skeleton.js', fileNames))
        return 'skeleton.js';
      else
        return getNextSkeletonFName({ fileNames });
    });
}

function getNextSkeletonFName({ fileNames, i = 1 }) {
  const fname = 'skeleton.' + i + '.js';
  if (!fp.includes(fname, fileNames))
    return fname;
  else
    return getNextSkeletonFName({ fileNames, i: i + 1 });
}
