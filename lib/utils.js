...
const cliLog = str => {
  if (state.isCli()) console.log(str)
}
...
const mapErrIdPropToString = transformWithKey(getErrIdsToString([]), {})
...
const reduceFirst = fp.curry((a, b) => vanillaReduce(b, a))
...
function getErrIdsToString(path) {
  return function errIdsToString(res, val, key) {
    ...
    if (fp.isPlainObject(val)) {
      path.push(key)
      res[key] = transformWithKey(getErrIdsToString(path), {}, val)
      path.pop()
    } else {
